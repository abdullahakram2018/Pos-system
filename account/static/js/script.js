...
function addItem(item_id, item_name, item_price,item_quantity,item_unit) {
  ...
  td_unit.id = 'aa'
  td_unit.innerHTML = item_unit;
  var td_quantity = document.createElement('td');
  
  let inp = document.createElement('input');
  inp.value =item_quantity;
  inp.className = 'form-control';
  inp.id = 'qty';
  var td_price = document.createElement('td');
  td_price.setAttribute('class', 'text-right');
  td_price.innerHTML =  item_price;
  var qty = document.getElementById('aa');
  var td_am = document.createElement('td');
  td_am.setAttribute('class', 'text-right');
  td_am.innerHTML =  qty;

  total_price += item_price;
  td_quantity.appendChild(inp);
